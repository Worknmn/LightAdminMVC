...
<form class="form-offerpropertytypes-edit" action="<?php echo URL . 'offerpropertytypes/update/'?>" method="post">
  ...
  <div class="form-row">
    <div class="col-md-12 mb-3">
      <div><label for="validationDefault05">Привязать к категории</label></div>
      <div><select name="offercategoryid[]"  multiple>
        <?php foreach ($OCs as $inst) { ?>
        <option value="<?php if (isset($inst->id)) echo $inst->id;?>"<?php if (isset($inst->select)AND($inst->select)) echo " selected";?>><?php if (isset($inst->name)) echo $inst->name;?></option>
        <?php } ?>
      </select></div>
    </div>
  </div>
